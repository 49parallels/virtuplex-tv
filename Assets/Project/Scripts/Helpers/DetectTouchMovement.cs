 ...
 *
 * Did some tweaking in multi-touch movement to get proper
 * scaleFactor for scaling purpose
 */
public class DetectTouchMovement : MonoBehaviour {
	...
	static public float pinchFactor = 0;
	...
	static public void Calculate () {
		...
		if (Input.touchCount == 2) {
			...
			if (touch1.phase == TouchPhase.Moved || touch2.phase == TouchPhase.Moved) {
				...
				if(Mathf.Approximately(prevDistance, 0)) return;
				
				pinchDistanceDelta = pinchDistance - prevDistance;
				
				// added pinchFactor ratio for proper scaling values
				pinchFactor = prevDistance / pinchDistance;
